...
public class SonyHeadphonesTests
{
    ...
    {
        // 4-bayt uzunluk başlıklı Sony MDR paketi
        ...
        packet[7] = 80;   // Pil seviyesi: %80
        packet[8] = 0x01; // Şarjda: 1
        packet[9] = 0x0C; // EOP
    ...
    public void TryParseSonyPacket_WF1000XM4_TwsBattery_ParsesLeftRightCaseAndChargingFlags()
    {
        ...
        packet[7] = 70;   // Sol kulaklık: %70
        packet[8] = 85;   // Sağ kulaklık: %85
        packet[9] = 95;   // Kutu: %95
        packet[10] = 0x05; // Şarj bitleri: bit 0 (Sol) ve bit 2 (Kutu) -> 0x05
        packet[11] = 0x0C; // EOP
    ...
    {
        // Bazı Sony modellerinde pil seviyesi 0-10 basamak olarak raporlanır (örn: 8 -> %80)
        byte[] packet = new byte[8];
        packet[0] = 0x0C;
        packet[1] = 0x00;
        packet[2] = 0x00;
        packet[3] = 0x02; // 2-bayt uzunluk formatı
        packet[4] = 0x02; // Function
        packet[5] = 7;    // Basamak 7 -> %70
        packet[6] = 0x00; // Deşarjda
        packet[7] = 0x0C;
    ...
    public void TryParseSonyPacket_CorruptedOrShort_ReturnsFalse()
    {
        ...
        Assert.False(AppSony.TryParseSonyPacket([0x01, 0x02, 0x03, 0x04, 0x05, 0x06], out _)); // SOM yok
    }
